...
export function KitchenForm({ 
  ...
  onSuccess?: () => void;
  initialData?: { itemName: string; quantity: number; unit: string; id?: string };
}) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const data = {
      ...
      status: "In Progress",
      category: "Daily Kitchen"
    };

    const url = isEditing 
      ? `/kitchen/${initialData?.id}`
      : "/kitchen";
    ...
    try {
      ...
      toast.success(isEditing ? "Kitchen log updated" : "Kitchen entry recorded");
    ...
    <form onSubmit={handleSubmit} className="space-y-6 pt-4 animate-fade-in">
      <div className="space-y-2">
        ...
        <Input
          ...
          placeholder="e.g. Rice Consumption, Vegetable Procurement"
          required
      ...
      >
        {loading ? "Saving..." : isEditing ? "Update Entry" : "Log Kitchen Activity"}
      </Button>
    </form>
  );
}
